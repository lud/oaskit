defmodule Oaskit.Plugs.ValidateRequest do
  ...
  * Body validation
  ...
  def call(conn, opts) do
    ...
    result =
      with {:ok, operation_id} <- fetch_operation_id(conn, controller, action),
        ...
        {:ok, merge_private(conn1, private)}
      else
        err -> {:error, err}
      end

    # This extra step is just for dialyzer. Otherwise we would call on_error
    # directly in the with's else.
    case result do
      {:ok, conn} -> conn
  ...
  defp on_error(conn, err, opts) do
    case err do
  ...
  defp merge_private(%Plug.Conn{private: %{oaskit: map} = private} = conn, new) do
    %{conn | private: %{private | oaskit: Map.merge(map, new)}}
  end

  defp merge_private(conn, new) do
    Plug.Conn.put_private(conn, :oaskit, new)
  end
