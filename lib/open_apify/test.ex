defmodule OpenApify.Test do
  alias OpenApify.Parsers.HttpStructuredField
  alias OpenApify.Plugs.ValidateRequest
  ...
  `MyAppWeb.ConnCase` test helper:
      ...
      defmodule MyAppWeb.ConnCase do
        ...
        def valid_response(conn, status) do
          OpenApify.Test.valid_response(MyAppWeb.OpenAPISpec, conn, status)
        end
      end
  ...
  def valid_response(spec_module, %Plug.Conn{} = conn, status) when is_integer(status) do
    ...
    operation_id =
      case get_in(conn.private, [:open_apify, :operation_id]) do
        nil ->
          raise """
          the connection was not validated by OpenApify.Plugs.ValidateRequest

          This may happen if no pipeline with the OpenApify plugs is defined for
    ...
    {validations, jsv_root} = OpenApify.build_spec!(spec_module, responses: true)
