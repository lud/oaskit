defmodule OpenApify.Spec.Response do
  ...
  JSV.defschema(%{
    ...
    properties: %{
      ...
      content: %{
        type: :object,
        additionalProperties: OpenApify.Spec.MediaType,
        description: "A map of potential response payloads by media type."
      },
      links: %{
        type: :object,
        additionalProperties: %{anyOf: [OpenApify.Spec.Reference, OpenApify.Spec.Link]},
  ...
  def normalize!(data, ctx) do
    ...
    |> normalize_subs(
      headers: {:map, {:or_ref, OpenApify.Spec.Header}},
      content: {:map, OpenApify.Spec.MediaType},
      links: {:map, {:or_ref, OpenApify.Spec.Link}}
    )
    |> collect()
  end
