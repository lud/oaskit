defmodule OpenApify.Controller do
  alias OpenApify.Spec.Operation
  alias OpenApify.Spec.Parameter
  alias __MODULE__
      ...
      defmodule MyAppWeb do
        def controller do
          quote do
            ...
            use OpenApify.Controller # <-- Add it there once for all
  ...
  defmacro __using__(opts) do
    quote bind_quoted: binding() do
      ...
      Module.register_attribute(__MODULE__, :open_apify_parameters, accumulate: true)
      Module.register_attribute(__MODULE__, :open_apify_tags, accumulate: true)
      Module.register_attribute(__MODULE__, :open_apify_operations, accumulate: true)
  ...
  with the `#{inspect(OpenApify.Plugs.ValidateRequest)}` plug automatically.
  ...
  * `:examples` - A list of examples.
  ...
  Parameters are stored into `conn.private.open_apify.path_params` and
  `conn.private.open_apify.query_params`. They do not override the `params`
  ...
  The body can be retrieved in `conn.open_apify.private.body_params`.
  ...
  * `:required` - A boolean. When `false`, the body can be missing and will not
    be validated. In that case, `conn.open_apify.private.body_params` will be
  ...
  **Important**, when using the shortcut, we chose to automatically define the
  ...
  A short form using a module schema:
      ...
      def create_user(conn, _params) do
        case Users.create_user(conn.private.open_apify.body_params) do
          # ...
        end
      end
  ...
  tried last by `#{inspect(Plug.Parsers)}`, as well as open_apify when choosing
  ...
  * `:content` - A mapping of content-type to media type objects, exactly as in
  ...
  defmacro operation(action, false) do
    quote do
      @open_apify_operations {unquote(action), false, nil}
    end
  end
  ...
  defmacro operation(action, spec) when is_atom(action) and is_list(spec) do
    ...
    quote bind_quoted: binding() do
      ...
      shared_parameters =
        :lists.reverse(Module.get_attribute(__MODULE__, :open_apify_parameters, []))

      shared_tags =
        :lists.flatten(:lists.reverse(Module.get_attribute(__MODULE__, :open_apify_tags, [])))
      ...
      @open_apify_operations {action, operation, verb}
    end
  end

  @doc """
  This macro allows OpenApify to validate request bodies, query and path
  ...
  defmacro use_operation(action, operation_id, opts \\ []) do
    ...
    quote bind_quoted: binding() do
      ...
      @open_apify_operations {action, {:use_operation, to_string(operation_id)}, verb}
    end
  end
  ...
  defmacro parameter(name, opts) when is_atom(name) do
    ...
    quote bind_quoted: binding() do
      @open_apify_parameters Parameter.from_controller!(name, opts)
    end
  end
  ...
  defmacro __before_compile__(env) do
    open_apify_operations = Module.delete_attribute(env.module, :open_apify_operations) || []
    _ = Module.delete_attribute(env.module, :open_apify_parameters)
    validate_duplicate_actions!(open_apify_operations, env)

    clauses =
      Enum.map(open_apify_operations, fn {action, operation, verb} ->
        case operation do
          false ->
    ...
    quote do
      @doc false
      def __open_apify__(kind, action, arg)

      unquote(clauses)

      # undef catchall
      def __open_apify__(kind, action, arg) do
        :__undef__
      end
    end
  end
  ...
  def _ignore_action(action) do
    quote do
      def __open_apify__(_kind, unquote(action), _verb) do
        :ignore
      end
    end
  end
  ...
  def _define_operation(action, %Operation{} = operation, verb) when is_atom(action) do
    ...
    quote bind_quoted: binding() do
      ...
      def __open_apify__(:operation, unquote(action), unquote(match_verb)) do
      ...
      def __open_apify__(:operation_id, unquote(action), unquote(match_verb)) do
  ...
  # able to match on it.
  defp validate_duplicate_actions!(open_apify_operations, env) do
    bad_cases =
      open_apify_operations
  ...
  Returns the validated parameter from `conn.open_apify.private.path_params` if
  found, or the default value.
  """
  def path_param(%Plug.Conn{} = conn, name, default \\ nil) do
    case conn do
      %{private: %{open_apify: %{path_params: %{^name => value}}}} -> value
      _ -> default
    end
  end
  ...
  Returns the validated parameter from `conn.open_apify.private.query_params` if
  found, or the default value.
  """
  def query_param(%Plug.Conn{} = conn, name, default \\ nil) do
    case conn do
      %{private: %{open_apify: %{query_params: %{^name => value}}}} -> value
      _ -> default
    end
  end
  ...
  def body_params(%Plug.Conn{} = conn) do
    conn.private.open_apify.body_params
  end
end
