defmodule OpenApify.ErrorHandler.Default do
  alias JSV.Codec
  ...
  * `#{Status.code(@status_parameters_errors)}`
  ...
  `#{inspect(OpenApify.Plugs.ValidateRequest)}` plug.
  """

  @behaviour OpenApify.ErrorHandler

  def handle_error(conn, reason, opts) do
    operation_id = conn.private.open_apify.operation_id
  ...
  @css_file :code.priv_dir(:open_apify) |> Path.join("assets/error.min.css")
