defmodule OpenApify.Web.ParamTest do
  use OpenApify.ConnCase, async: true
        ...
        get_reply(
          ...
          fn
            conn, _params ->
              ...
              assert %{} == conn.private.open_apify.query_params
              assert %{} == conn.private.open_apify.body_params
    ...
    test "valid param", %{conn: conn} do
      conn =
        get_reply(conn, ~p"/generated/params/some-slug/t/dark", fn conn, _params ->
          assert %{theme: :dark} = conn.private.open_apify.path_params
          json(conn, %{data: "ok"})
        end)
    ...
    test "both valid path params", %{conn: conn} do
      conn =
        get_reply(conn, ~p"/generated/params/some-slug/t/dark/c/red", fn conn, _params ->
          assert %{theme: :dark, color: :red} = conn.private.open_apify.path_params
          json(conn, %{data: "ok"})
        end)
    ...
    test "both scope and path params valid", %{conn: conn} do
      conn =
        get_reply(conn, ~p"/generated/params/some-slug/s/square/t/light", fn conn, _params ->
          assert %{shape: :square, theme: :light} = conn.private.open_apify.path_params
          json(conn, %{data: "ok"})
        end)
        ...
        get_reply(
          ...
          fn
            conn, params ->
              ...
              # open_apify data is properly cast
              assert %{slug: "some-slug", shape: :square, theme: :light, color: :red} ==
                       conn.private.open_apify.path_params

              assert %{shape: 10, theme: 20, color: 30} == conn.private.open_apify.query_params
      ...
      conn =
        get_reply(conn, ~p"/generated/params/some-slug/s/square/t/light/c/red?shape=10", fn
          conn, params ->
            ...
            # open_apify data is properly cast
            assert %{shape: 10} == conn.private.open_apify.query_params
      ...
      conn =
        get_reply(
          ...
          fn conn, params ->
            ...
            # Assert that OpenApify properly casts the parameters
            assert %{
                     string_param: "hello",
                     boolean_param: true,
                     integer_param: 42,
