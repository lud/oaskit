defmodule OpenApify.Internal.ValidationBuilderTest do
  alias OpenApify.Internal.Normalizer
  alias OpenApify.Internal.ValidationBuilder
  ...
  alias OpenApify.TestWeb.PathsApiSpec
  ...
  test "build validations for request bodies" do
    assert {built, _} =
             ValidationBuilder.build_operations(@normal_sample_spec, %{
               responses: false,
               jsv_opts: OpenApify.default_jsv_opts()
             })
  ...
  test "duplicate operation ids" do
    defmodule DupAController do
      alias OpenApify.TestWeb.Helpers
  ...
  describe "using generated api spec" do
    test "params and bodies" do
      ...
      assert {built, _} = OpenApify.build_spec!(PathsApiSpec, cache: false)
    ...
    test "with responses" do
      assert {built, _} = OpenApify.build_spec!(PathsApiSpec, cache: false, responses: true)
  ...
  describe "using spec from maps" do
    test "basic build" do
      ...
      assert {built, _} = OpenApify.build_spec!(DeclarativeApiSpec, cache: false)
    ...
    test "with responses" do
      assert {built, _} = OpenApify.build_spec!(DeclarativeApiSpec, cache: false, responses: true)
